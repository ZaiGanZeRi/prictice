...
class S_tring
{
	public:
		S_tring();
		~S_tring(){cout<<"析构函数执行\n";delete []str;len=0;}//析构函数，避免内存泄露。 
		S_tring(const char *const ch); 
//		S_tring(unsigned short int); //声明一个无符号短整型构造函数,应放在私有区域中。 
...
//		char operator[](unsigned short int length)const;//可定义有无符号短整型 
        ...
        S_tring operator+(const S_tring&s);//声明+的重载函数 
        void operator+=(const S_tring&s);//声明+＝的重载函数 该函数没有返回值。 
	private:
		S_tring(unsigned short int); //声明一个无符号短整型构造函数,应放在私有区域中。
		int len;
...
S_tring::S_tring(unsigned short int length)//length是字符串总长度 
{
	cout<<"带一个short int参数的构造函数\n";
	len=length;//length是字符串总长度 交给新建的len成员保存 
	str=new char[len+1];
	for (int i=0;i<=len;i++)
	{//for退出条件是i<=len，这里保证了将该数组的所有元素都填充为空字符。 
		str[i]='\0';//将该数组的所有元素都填充为空字符。 	 
	}
}
...
S_tring S_tring:: operator+(const S_tring&s)//按值返回 
{
	cout<<"operator+函数执行\n";
	int total=len+s.getlen();
	S_tring temp(total);//total的值是两个字符串的总长度。 
                        //temp拥有一个可容纳２个字符串的数组 
    int i,j;
    for (i=0;i<len;i++) 
    {
    ...
    {
    	temp[i]=s[j];//左边的值是可以修改的        
	}
//	temp[total]='\0';//将字符串结束标志赋给该数组最后一个元素。可省。 
    return temp; 
}                        

void S_tring:: operator+=(const S_tring&s)//按值返回 
{
	cout<<"operator+=函数执行\n";
	int total=len+s.getlen();
	S_tring temp(total);//total的值是两个字符串的总长度。 
...
//	temp[total]='\0';//将字符串结束标志赋给该数组最后一个元素。可省。 
    *this=temp; //赋给执行调用该函数的对象也就是this指针指向的对象 
}  
